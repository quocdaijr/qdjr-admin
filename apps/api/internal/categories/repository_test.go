...
// testFixture seeds two categories with unique slugs/ids so the test can run
...
func newFixture(t *testing.T) *testFixture {
	...
	slugA := "cat-a-" + uuid.New().String()[:8]
	slugB := "cat-b-" + uuid.New().String()[:8]

	descA := "alpha description"
	_, err = pool.Exec(ctx,
		`insert into public.categories (id, slug, name, description) values ($1, $2, $3, $4)`,
		idA, slugA, "Alpha", descA,
	)
	require.NoError(t, err)
	// B has nil description to assert pointer behaviour.
	_, err = pool.Exec(ctx,
		`insert into public.categories (id, slug, name) values ($1, $2, $3)`,
		idB, slugB, "Bravo",
	)
	require.NoError(t, err)
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(bg, `delete from public.categories where id = any($1)`,
			[]uuid.UUID{idA, idB})
	})
...
func TestRepository_List(t *testing.T) {
	...
	// Find our two slugs in the output and verify ordering by name (Alpha < Bravo).
	idxA, idxB := -1, -1
	for i, c := range out {
		switch c.Slug {
		case f.slugA:
			...
			assert.Equal(t, "alpha description", *c.Description)
			assert.Equal(t, "Alpha", c.Name)
		case f.slugB:
			idxB = i
			assert.Nil(t, c.Description)
			assert.Equal(t, "Bravo", c.Name)
		}
	}
