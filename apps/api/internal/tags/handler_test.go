...
// stubReader is a Reader test double for the tags repository.
...
func TestList_OK(t *testing.T) {
	...
	r.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/v1/tags", nil))
	...
	assert.Equal(t, "golang", body.Data[0].Slug)
	require.NotNil(t, body.Data[0].Description)
	assert.Equal(t, "go language", *body.Data[0].Description)
}
...
func TestPostsBySlug_OK(t *testing.T) {
	now := time.Now().UTC()
	tag := Resource{ID: uuid.New(), Slug: "golang", Name: "Golang", CreatedAt: now, UpdatedAt: now}
	reader := &stubReader{slugOut: tag}
	...
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/v1/tags/golang/posts?page=2&perPage=200&q=foo", nil))
	require.Equal(t, http.StatusOK, rr.Code)
	...
	assert.Equal(t, "golang", reader.lastSlug)
	assert.Equal(t, "golang", pr.lastFilter.TagSlug)
	assert.Equal(t, "", pr.lastFilter.CategorySlug, "tag handler must not set CategorySlug")
...
func TestPostsBySlug_NotFoundEnvelope(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/v1/tags/missing/posts", nil))
	...
	// posts reader must not be called when tag doesn't exist.
...
func TestPostsBySlug_EmptyButExisting(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/v1/tags/golang/posts", nil))
