...
// RegisterAdmin mounts the admin tags CRUD on group g (already wrapped in
// RequireAuth by the router). Per-route taxonomy:write checks added inline.
...
func adminGetHandler(repo AdminWriter) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		t, err := repo.GetByID(c.Request.Context(), id)
		switch {
