...
// RegisterPublic mounts the public tags endpoints on the given group.
//
// postsReader is used to resolve the posts-by-tag endpoint without
// duplicating the post listing logic.
func RegisterPublic(g *gin.RouterGroup, repo Reader, postsReader posts.Reader) {
	g.GET("/tags", listHandler(repo))
	g.GET("/tags/:slug/posts", postsBySlugHandler(repo, postsReader))
}
	...
	return func(c *gin.Context) {
		...
		if _, err := repo.GetBySlug(c.Request.Context(), slug); err != nil {
			if errors.Is(err, ErrNotFound) {
				apphttp.Err(c, http.StatusNotFound, "NOT_FOUND", "tag not found")
				return
			}
			apphttp.Err(c, http.StatusInternalServerError, "INTERNAL", err.Error())
			return
