...
export function TagsTable() {
  ...
  const queryKey = ['tags', page, PER_PAGE, q] as const;
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (q) params.set('q', q);
      return apiList<Tag>(`/v1/admin/tags?${params.toString()}`);
    },
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => apiDelete(`/v1/admin/tags/${id}`),
    onSuccess: () => {
    ...
    onError: (err) => {
      const message =
        err instanceof ApiError
          ? err.code === 'IN_USE'
            ? 'Tag is in use by posts and cannot be deleted'
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => setEditTarget(tag)}>
                          Edit
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
